...
function Review() {
  ...
  const openEmailModal = () => {
    ...
    const title = meetingData ? formatMeetingTitle(meetingData.title) : `Meeting #${parsedMeetingId}`
    const date = meetingData ? formatDate(meetingData.created_at) : ''
    const lines = []

    lines.push(`Hi team,`)
    lines.push(``)
    lines.push(`Here's a summary of our meeting: ${title}${date ? ` (${date})` : ''}.`)

    if (sm?.summary) {
      lines.push(``)
      lines.push(`Summary`)
      lines.push(`-------`)
      lines.push(sm.summary)
    }

    const todoItems = tasks.filter((t) => t.type === 'todo' && t.status === 'approved')
    if (todoItems.length > 0) {
      lines.push(``)
      lines.push(`Action Items`)
      lines.push(`------------`)
      todoItems.forEach((t) => lines.push(`• ${t.title}`))
    }

    const parkingItems = tasks.filter((t) => t.type === 'parking' && t.status === 'approved')
    if (parkingItems.length > 0) {
      lines.push(``)
      lines.push(`Parking Lot`)
      lines.push(`-----------`)
      parkingItems.forEach((t) => lines.push(`• ${t.title}`))
    }

    const scheduleItems = tasks.filter((t) => t.type === 'schedule' && t.status === 'approved')
    if (scheduleItems.length > 0) {
      lines.push(``)
      lines.push(`To Schedule`)
      lines.push(`-----------`)
      scheduleItems.forEach((t) => lines.push(`• ${t.title}`))
    }

    lines.push(``)
    lines.push(`Generated by MeetingMind AI`)
  ...
  const emailSubject = meetingData
    ? `Meeting Summary — ${formatMeetingTitle(meetingData.title)}`
    : 'Meeting Summary'
  ...
  return (
    <div className="rv-page">
        ...
        <div className="rv-summary">
          <div className="rv-summary-header">
            <div className="rv-summary-title-row">
              ...
              {meetingData?.summary && (
                <button className="rv-email-btn" onClick={openEmailModal}>
      ...
      {emailModal && (
        <div className="rv-modal-overlay" onClick={() => setEmailModal(false)}>
          <div className="rv-modal" onClick={(e) => e.stopPropagation()}>
            <div className="rv-modal-header">
              ...
              <button className="rv-modal-close" onClick={() => setEmailModal(false)}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
                </svg>
              </button>
            </div>
            <div className="rv-modal-subject">
              <span className="rv-modal-subject-label">Subject</span>
              <span className="rv-modal-subject-value">{emailSubject}</span>
            </div>
            <textarea
            ...
            <div className="rv-modal-footer">
              <button
                className={`rv-modal-btn rv-modal-btn--copy${emailCopied ? ' rv-modal-btn--copied' : ''}`}
                onClick={() => {
              ...
              <a
                className="rv-modal-btn rv-modal-btn--mail"
